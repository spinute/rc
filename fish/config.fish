...
set -x PATH (brew --prefix mysql@5.6)/bin $PATH
set -x PATH $PATH $ANDROID_HOME/tools
set -x PATH $PATH $ANDROID_HOME/tools/bin
set -x PATH $PATH $ANDROID_HOME/platform-tools
set -x FZF_LEGACY_KEYBINDINGS 0

set -x LDFLAGS -L(brew --prefix mysql@5.6)/lib:/usr/local/opt/openssl/lib
set -x CPPFLAGS -I(brew --prefix mysql@5.6)/include:(brew --prefix openssl)/include
